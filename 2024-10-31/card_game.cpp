...
void solve(int test){
     ...
     fr(i,0,n) cin>>v[i];
     if(n == 1) {
        cout<<max(0,(int) v[0])<<endl;
        return;
     }
     if(n == 2){
        cout<<max({(int) 0,(int)v[0],int(v[0] + v[1])})<<endl;
        return;
     }
     d[n-1] = (v[n-1] >0 ? v[n-1]:0);
     for(int i = n-2;i>=0;i--){
        d[i] = d[i+1] + (v[i] >0 ? v[i]:0);


     }
      ll ans = max(v.back(),(long long)0);
     for(int i = 1;i<n;i++){
        int ind = i-1;
        if(i == 1){
                ans = max(ans,v[ind] + d[i]);
        }
        else if(i == 2){
              ans = max(ans,d[i] + v[ind] + v[ind-1]);
        }
        else {
                 ans = max(ans,v[ind] + d[i]);
        }
