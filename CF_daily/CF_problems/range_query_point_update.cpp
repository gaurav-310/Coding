...
vecl lazy;
vecl tr;
void propogate(ll l,ll r,ll ind){
       if(lazy[ind] != 0){
        dbg(ind);
             tr[ind] = lazy[ind] + tr[ind];
...
void modify(ll l,ll r,ll ind,ll i,ll j){
     if(l == r){
        tr[ind] = 1;
        return;
     }
    ...
    ll mid = (l + r)/ 2;
    modify(l , mid,2*ind + 1,i,j);
    modify(mid + 1,r,2*ind + 2, i, j);
    tr[ind] = tr[2*ind + 1] + tr[2*ind + 2];

    return;
    
}
ll value(ll l,ll r,ll ind, ll i){
    if(l == r and r == i){
        // tr[ind] = 1;
        return tr[ind];
    }
    ll mid = (l + r) / 2;
    ll a = value(l,mid,2*ind + 1,i);
    ll b = value(mid+1,r,2*ind + 2,i);
