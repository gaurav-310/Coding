...
void solve(int test){
  ...
  cin>>n>>m;
    ...
    vector<vecl> rows(n,vecl(m,0)),cols(m,vecl(n,0)),row_b(n,vecl(m,0)),col_b(m,vecl(n,0));
    fr(i,0,n){
      fr(j,0,m){
        cin>>rows[i][j];
        cols[j][i] = rows[i][j];
    ...
    auto sorting = [&](vector<vecl> &a){
           for(auto &it:a) sort(it.begin(),it.end()); 
           sort(all(a));
    };
    auto check = [](vector<vecl> &a,vector<vecl> &b,bool ok)-> bool{
    ...
    dbg(cols);
    dbg(col_b);
    if(!check(rows,row_b,false) or !check(cols,col_b,true)){
      cout<<"NO"<<nl;
      return;
    }
    cout<<"YES"<<nl;
