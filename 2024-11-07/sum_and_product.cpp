...
ll calculate_d(ll x,ll y){
    
    long double temp = ((x*x) - 4*y);
    if(temp  < 0) return -1;
    long double d= sqrt((temp));
    ll int_d = ll(d);
    if(int_d != d) return -1;
    return d;
...
void solve(int test){
    ...
    fr(i,0,q){
        ll x,y;
        cin>>x>>y;
        ll d= calculate_d(x,y);
        if(d == -1){
            cout<<0<<" ";
            continue;
        }
        ll first_value = (x + d)/2;
        ll second_value = (x - d)/2;
        if(first_value == second_value){
            cout<<(m[first_value] * (m[first_value] -1 ))/2<<" ";
            continue; 
        }
        ll ans = m[first_value] * m[second_value];
        cout<<ans<<" ";
    }
    cout<<nl;
    return;
